 ...
 * @param optionsInstId - Options instrument ID - REQUIRED
 */
export async function fetchPrices(baseCurrency: string, optionsInstId: string): Promise<{
  ...
  try {
    const [spotPrice, optionPrice] = await Promise.all([
      fetchSpotPrice(baseCurrency),
      fetchOptionsPrice(optionsInstId)
    ]);
